...
class EYiiPdf extends CApplicationComponent
{
	...
	public function renderPartial($view, $params)
	{
		...
		$html = $controller->renderPartial($view, $params, true, false);
		return $controller->renderPartial('/docs/layout', array('data' => $html), true);
	}
	...
	public function Output($view,$params)
	{
		...
		$pdf->WriteHTML($this->renderPartial($view,$params,true));
		return $pdf->Output("","S");
	}

	public function Output2($view,$params)
	{
		$pdf = $this->mpdf();
		$stylesheet = file_get_contents(Yii::getPathOfAlias('webroot.css').'/pdf2.css');
		$pdf->WriteHTML($stylesheet, 1); 
		$pdf->WriteHTML($this->renderPartial($view,$params,true));
		return $pdf->Output("","S");
	}

	public function WriteOutput($html,$params)
	{
		$pdf = $this->mpdf();
		$stylesheet = file_get_contents(Yii::getPathOfAlias('webroot.css').'/pdf2.css');
		$pdf->WriteHTML($stylesheet, 1); 
	...
	public function Stream($view,$params,$name)
	{
		$pdf = $this->mpdf();
		$stylesheet = file_get_contents(Yii::getPathOfAlias('webroot.css').'/pdf.css');
		$pdf->WriteHTML($stylesheet, 1); 
	...
	public function Download($view,$params,$name)
	{
		$pdf = $this->mpdf();
		$stylesheet = file_get_contents(Yii::getPathOfAlias('webroot.css').'/pdf.css');
		$pdf->WriteHTML($stylesheet, 1); 
	...
	public function Download2($view,$params,$name)
	{
		$pdf = $this->mpdf();
		$stylesheet = file_get_contents(Yii::getPathOfAlias('webroot.css').'/pdf2.css');
		$pdf->WriteHTML($stylesheet, 1); 
